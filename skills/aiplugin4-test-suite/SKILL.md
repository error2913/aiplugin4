...
3. 先记录「后端」待改字段的原始值（快照）。后端页签含 5 个 URL 字段（流式输出/图片转base64/联网搜索/用量图表/论坛地址）加 2 个文本字段（论坛API Token/论坛签名密钥），字段名显示在表单内容区。md/html 渲染（md-html-render）已移入「工具 → MCP服务器配置」，不在后端页签。
