...
| TOOL-02 | `.ai tool help get_time` | U | `get_time`、`必需参数` | |
| TOOL-03 | `.ai tool help 不存在的工具` | U | `没有这个工具函数` | 错误路径 |
| TOOL-04 | `.ai tool on` | I | `已开启全部工具函数` | |
| TOOL-05 | `.ai tool off` | I | `已关闭全部工具函数` | |
| TOOL-06 | `.ai tool on get_time` | I | `已开启工具函数 get_time` | |
| TOOL-07 | `.ai tool off get_time` | I | `已关闭工具函数 get_time` | |
| TOOL-08 | `.ai tool call get_time` | M | `返回内容`、`时间` | 需骰主；依赖系统时间 |
| TOOL-09 | `.ai tool call 不存在的函数` | M | `调用函数失败:未注册的函数` | 错误路径 |
| TOOL-10 | `.ai tool on <禁止调用的函数>` | I | `不被允许开启` | 仅当知道"禁止调用的函数"配置项内容时测，否则 SKIP(env) |

说明：

- `tool call` 有调用副作用（如 `ban`/`whole_ban`/`record`/`text_to_sound`），只测无副作用的 `get_time` 与未注册函数错误路径。
