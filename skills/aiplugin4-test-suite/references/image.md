...
| IMG-04 | `.ai img itt [CQ:image,file=<公开图片URL>]` | M | `CQ:image` | 依赖「多模态模型」配置（use=image-understanding）；未配置则 SKIP(env) |
...
- `itt` 的真实识别依赖「模型」页签的「多模态模型」配置（use=image-understanding）且「是否开启识图模型」开关打开；无配置时只测错误路径。
