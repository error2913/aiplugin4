...
仓库内附带可直接加载的完整示例插件 [`examples/use-aiplugin4.js`](../examples/use-aiplugin4.js)(提供 `.ai4chat`/`.ai4run`/`.ai4agent`/`.ai4status` 四个命令,覆盖 `chat`/`run`/`getAgent` 三种调用方式),可复制到海豹中加载体验。
