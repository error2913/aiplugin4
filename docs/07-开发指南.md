...
- 在 shell 中不要直接写中文;涉及中文参数或输出时使用 UTF-8 编码(如用 `[System.Text.Encoding]::UTF8` 处理字节、将中文放入 UTF-8 文件/环境变量)。
...
- `release.yml`:推送 `v*` 标签时自动发版——verify(校验标签与 `VERSION`/`update.ts` 一致,并跑 `backends/launcher.py` 的 Linux 校验)→ `node scripts/build-release.js` 打包(本体 JS + 本体豹包 + 完整豹包)+ `python3 backends/launcher.py package` 打包后端压缩包 → 用 `SEALPACK_TOKEN` 分别发布两个豹包到 SealRepo → 从 `src/update.ts` 提取版本日志作为 GitHub Release 正文,附带两个豹包、后端压缩包与 `dist/aiplugin4.js`。
...
后端服务在 `backends/`(目录与文件均为英文命名),由 `backends/launcher.py` 一键管理(list/setup/start/stop/status/port/package/webui),Windows/Linux 通用;默认不启动任何后端,首次启动某后端时才自动安装其环境(独立 venv);发版时自动打包 `aiplugin4-backends-<版本>.zip`。详见 [08-相关后端项目](08-相关后端项目.md)。
