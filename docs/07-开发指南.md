...
`run` 的可选参数 `options`:`agentName`(智能体名,缺省 `*`)、`reason`(触发原因,缺省「外部插件调用」)、`toolChoice`(强制工具选择策略,缺省交给模型)。

`registerTool` 的可选参数 `options`:`sensitive`(敏感工具)、`sessionType`(any/group/user)、`callBack`(是否回调对话:工具结果写回上下文并继续生成,缺省 true;false 时工具静默执行、结果不回调)、`solve`(执行函数,返回给 AI 的文本)。
