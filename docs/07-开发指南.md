...
后端服务在 `backends/`(目录与文件均为英文命名),由 `backends/launcher.py` 一键管理(list/enable/disable/setup/start/stop/status/package),Windows/Linux 通用;发版时自动打包 `aiplugin4-backends-<版本>.zip`。详见 [08-相关后端项目](08-相关后端项目.md)。
