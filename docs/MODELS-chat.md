...
插件内置的自动识别表只覆盖部分经典模型，且可能滞后于各平台上线 / 退役节奏。需要其他模型或服务商时，从本文档复制完整 TOML，粘贴到 SealDice →「插件设置」→ aiplugin4 →「模型」→「纯文本模型」中，替换 `api_key` 等示例值即可。每行一个完整 TOML，多个模型用换行分隔；默认纯文本模型取列表第一项。
