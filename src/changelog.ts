...
export const changelog: { [version: string]: string } = {
    "4.20.0": `## 新功能
- 新增 .ai model：查看/设置当前会话使用的模型（按会话覆盖）；可选中多模态模型里 use 含 chat 的条目作为会话模型，选中后该会话按多模态处理（上下文图片直接传给模型）

## 配置变更
- 「图片模型」更名为「多模态模型」：列表内模型一律按多模态处理——use 含 image-understanding 用于识图，use 含 chat/compression/summarization/judge 可当对应用途的对话模型使用（上下文图片直接传模型，不再转文本标签）；同名模型若同时出现在「对话模型」列表，按对话模型（纯文本）优先
