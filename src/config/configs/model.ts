// 模型配置：对话/多模态/嵌入模型 TOML 解析，并同步 Model 静态列表
...
top_p = 1                           # 可选
max_tokens = 8192                   # 可选`
        ], '每行一个模型（TOML）。必填：name（模型名）、api_key（API 密钥）、use（用途，可多个）。可选：provider（服务商，省略时按模型名自动识别：deepseek/openai/google/zhipu/alibaba/anthropic/moonshot/xai/mistral/siliconflow）、base_url（API 地址，省略时按服务商取默认）、body（请求参数覆盖）。use 可选值：chat（普通对话）/compression（消息压缩）/summarization（记忆总结）/judge（群聊插话评分，未单独配置时回退到 chat 模型）。默认对话模型取列表第一项；body 未配置时使用 max_tokens=8192、stop=null、stream=false。下方默认值即完整示例，可直接修改。完整格式指导与各平台模型示例见 https://github.com/error2913/aiplugin4/blob/main/docs/MODELS-chat.md （仓库文档）。', "模型");
        seal.ext.registerBoolConfig(ext, "是否开启识图模型", false, "总开关，默认关闭。\n只影响图片识别/图片转文字（image-understanding）；多模态对话不受此开关影响", "模型");
...
temperature = 1                     # 可选
max_tokens = 2048                   # 可选`
        ], '每行一个多模态模型（TOML）。必填：name（模型名）、api_key（API 密钥）、use（用途）。可选：provider（服务商，省略时按模型名自动识别：zhipu/alibaba/openai/google/siliconflow）、base_url（API 地址，省略时按服务商取默认）、body（请求参数覆盖）。use 可选值：image-understanding（图片理解/图片转文字）、chat/compression/summarization/judge（把该模型当作对应用途的对话模型使用，上下文里的图片会以图片内容直接传给模型，不再转成文本标签；.ai model 可选中 use 含 chat 的条目作为会话模型）。本列表内的模型一律按多模态处理；同名模型若也出现在对话模型列表，按对话模型（纯文本）处理。body 未配置时使用 max_tokens=2048、stop=null、stream=false。下方默认值即完整示例，可直接修改。完整格式指导与各平台模型示例见 https://github.com/error2913/aiplugin4/blob/main/docs/MODELS-image.md （仓库文档）。', "模型");
