...
api_key = "sk-xxxx"
use = ["chat"]`
        ], '每行一个模型（TOML），name/api_key/use 必填；provider/base_url 可省略（自动识别 deepseek/openai/google/zhipu/alibaba）；默认对话模型取列表第一项；可选 [body] 覆盖请求参数（如 temperature、max_tokens）', "模型");
...
api_key = "sk-xxxx"
use = ["image-understanding"]`
        ], '每行一个图片模型（TOML），name/api_key/use 必填；provider/base_url 可省略（自动识别 zhipu/alibaba/openai）；use 填 image-understanding', "模型");
...
api_key = "sk-xxxx"
use = ["text-embedding"]`
        ], '每行一个嵌入模型（TOML），name/api_key/use 必填；provider/base_url 可省略（自动识别 alibaba/openai）；输出维度需与“向量维度”配置一致', "模型");
    }
