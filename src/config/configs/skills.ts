...
export default class SkillsConfig {
    static register() {
        ...
        seal.ext.registerTemplateConfig(ext, "技能配置", [...SEALDICE_COMMAND_SKILLS, ...OB11_API_SKILLS], "每条配置项一个技能，仅支持标准 SKILL.md 格式：以 --- 开头的 YAML frontmatter 里写 name（必填）/description（可选）/platform（可选平台白名单数组，如 [QQ, DISCORD]，缺省=所有平台），正文为技能内容；默认包含当前 SealDice 核心命令、内置扩展命令及别名的调用帮助，统一说明 run_ext_command / run_core_command 的参数传递方式。修改后可用 .ai skill refresh 生效（或重载 JS）。AI 可通过 use_skill 工具按需调用", "技能");
    }
