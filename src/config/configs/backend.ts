...
export default class BackendConfig {
    static register() {
        ...
        seal.ext.registerStringConfig(ext, "网页读取", "http://127.0.0.1:46799/mcp", '网页读取后端 MCP 地址（aiplugin4-backends 的 web-read，Streamable HTTP，/mcp 端点），供读取网页内容工具使用', "后端");
        ...
        seal.ext.registerStringConfig(ext, "md和html图片渲染", "http://127.0.0.1:37632/mcp", 'Markdown/HTML 渲染为图片的后端 MCP 地址（aiplugin4-backends 的 md-html-render，Streamable HTTP，/mcp 端点）', "后端");
