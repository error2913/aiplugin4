...
export default class ReplyConfig {
    static register() {
        ...
        seal.ext.registerBoolConfig(ext, "非流式分段发送延时", true, "仅非流式生效，消息间隔是否开启延时防止乱序", "回复");
        seal.ext.registerIntConfig(ext, "非流式分段发送基础延时/ms", 350, "仅非流式生效，从第二条消息开始每条发送前等待的毫秒数", "回复");
        seal.ext.registerIntConfig(ext, "非流式分段发送含图额外延时/ms", 250, "仅非流式生效，当消息包含图片时额外增加的等待毫秒数", "回复");
    ...
    static get() {
        return {
            ...
            NON_STREAM_SEGMENT_DELAY_ENABLED: seal.ext.getBoolConfig(ext, "非流式分段发送延时"),
            NON_STREAM_SEGMENT_DELAY_MS: seal.ext.getIntConfig(ext, "非流式分段发送基础延时/ms"),
            NON_STREAM_SEGMENT_IMAGE_EXTRA_DELAY_MS: seal.ext.getIntConfig(ext, "非流式分段发送含图额外延时/ms"),
