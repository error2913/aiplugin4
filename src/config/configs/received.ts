...
export default class ReceivedConfig {
    static register() {
        seal.ext.registerBoolConfig(ext, "接收图片", true, "是否接收并识别图片消息", "消息接收");
        seal.ext.registerBoolConfig(ext, "接收指令消息", false, "是否将指令消息计入上下文（指令仍会执行）", "消息接收");
        seal.ext.registerBoolConfig(ext, "接收骰子发送的消息", false, "是否处理机器人自己发送的消息", "消息接收");
