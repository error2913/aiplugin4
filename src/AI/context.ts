...
import { sendToolRespondCompressRequest } from "../service";
...
export class Context {
    ...
    async compressToolResponseIfNeeded(toolName: string, content: string, searchTarget: string = ''): Promise<string> {
        ...
        if (toolName === 'web_search' && searchTarget.trim()) {
            prompt += `\n\n搜索目标:\n${searchTarget.trim()}`;
        }

        const summary = (await sendToolRespondCompressRequest([
            { role: 'system', content: prompt },
            { role: 'user', content }
        ])).trim();

        if (!summary) {
            logger.warning(`工具响应压缩返回为空，保留原始响应: ${toolName}`);
            return content;
        }
        return summary;
    }
