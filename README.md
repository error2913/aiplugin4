...
![Version](https://img.shields.io/badge/Version-4.19.1-green)
...
| 知识库注入阈值(字符) | 已弃用：知识库改为按当前对话检索式注入，命中正文在 1500 字符预算内才注入（最多 5 个分块），否则预算优先用于条目索引（尽可能多列，最多 100 条）；模型用 knowledge_read 工具读取详情（保留该配置仅为兼容旧存档） |
...
- 知识库为配置驱动（Markdown 模板），不按角色加载；「启用知识库记忆」开关打开后修改「知识库」配置，修改后需重载 JS 生效，用 `.ai kb search <关键词>` / `.ai kb list` 验证；
