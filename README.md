...
![Version](https://img.shields.io/badge/Version-4.21.0-green)
...
| 通知事件白名单 | 每行一个事件类型；notify 大类下需单独写子类型才收录；poke 由原生处理不录入；事件文本过长时按「消息压缩阈值」压缩，完整原始数据由 `get_event_detail` 工具只读读取 |
...
| 工具响应压缩触发字数 | 工具返回结果超过该字数时压缩后再存入上下文，设为 0 不压缩（默认 10000）；web_search 压缩时附带搜索目标；压缩前原文保留，可由 `grep_tool_raw` / `read_tool_raw` 只读检索 |
