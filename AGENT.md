...
- `src/agent/`：智能体编排（run/runStream、对外 API `globalThis.aiplugin4`）；`src/agent/subagent/`：DSH 对齐子代理框架（委派/fork、后台 job、可续跑子代理、持久化与唤醒）。
- `src/context/`、`src/session/`：会话与上下文管理；`src/prompt/`：提示词组装。
- `src/tool/`：工具系统（注册表、MCP、skills、按需加载；内置工具在 `src/tool/tools/`，新增分组工具在 `pub/`、`subagent/` 子目录）。
- `src/config/`：配置注册/读取与静态常量；`src/model/`：模型注册表/api连接列表拉取与分类/模型规则请求模板/余额查询。
- `src/cmd/`：`.ai` 命令系统与权限；`src/pub/`：公开会话目录（`.ai pub` + `pub_read`/`pub_send` 工具）；`src/memory/`：记忆与知识库；`src/resource/`：资源；`src/utils/`：通用工具。
- `docs/`：项目知识库（01 概览 / 02 架构 / 04 工具系统 / 05 命令配置 / 07 开发指南 / 09 常见问题；12 子代理方案设计）。
...
6. **配置项描述要引导用户**：写清格式、必填/可选参数、示例；配置页签顺序以 `src/config/config.ts` 的 `configMap` 注册顺序为准（基础/模型/角色设定/上下文/消息接收/消息触发/回复/工具/MCP/技能/子代理/记忆/知识库/图片/后端/事件接收/资源/错误处理）。
...
1. 版本推进：`src/config/static_config/meta.ts` 的 `VERSION`、`header.txt` 的 `@version`、`package.json` 与 `package-lock.json` 的 `version`、`src/changelog.ts` 新增对应版本条目（`sealpack/info.toml` 的 `version` 由 `scripts/prepare-sealpack.js` 自动同步，发版提交一并保持一致），走 PR 合并到 main。
